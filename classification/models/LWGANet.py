...
class star_Block(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        # 确保mlp_ratio * dim是整数
        mlp_hidden_dim = int(dim // 3 * mlp_ratio)
        self.stage = stage
        self.dim_split = dim // 3
        self.dwconv = ConvBN(dim // 3, dim // 3, 7, 1, (7 - 1) // 2, groups=dim // 3, with_bn=True)
        self.f1 = ConvBN(dim // 3, mlp_hidden_dim, 1, with_bn=False)
        self.f2 = ConvBN(dim // 3, mlp_hidden_dim, 1, with_bn=False)
        self.g = ConvBN(mlp_hidden_dim, dim // 3, 1, with_bn=True)
        self.dwconv2 = ConvBN(dim // 3, dim // 3, 7, 1, (7 - 1) // 2, groups=dim // 3, with_bn=False)
        self.act = nn.ReLU6()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        # self.HSMSSD = HSMSSD(d_model=dim // 3, ssd_expand=ssd_expand,state_dim=state_dim)
        # self.trans = TransformerBlock(dim=dim // 3,
    ...
    def forward(self, x: Tensor) -> Tensor:
        # for training/inference
        shortcut = x.clone()
        x1, x2, x3 = torch.split(x, [self.dim_split, self.dim_split, self.dim_split], dim=1)
        # x1 = x1 + self.PA(x1)
        # 原 StarNet 的操作
        ...
        if self.stage == 2:
            ...
            x3 = self.norm(x3 + self.GA4(x3))
        else:
            x3 = self.norm(x3 + self.GA12(x3))
        x_att = torch.cat((x1, x2, x3), 1)
...
def starnet_m1(num_classes: int = 30, **kwargs):
    model = starnetm1(in_chans=3,
                    num_classes=num_classes,
                    stem_dim=96,
                    depths=(2, 2, 4, 2),
                    att_kernel=(11, 11, 11, 11),
...
def starnet_m2(num_classes: int = 30, **kwargs):
    model = starnetm1(in_chans=3,
                    num_classes=num_classes,
                    stem_dim=96,
                    depths=(3, 3, 9, 3),
...
def starnet_T2(num_classes: int = 30, **kwargs):
    model = starnett2(in_chans=3,
                    num_classes=num_classes,
                    stem_dim=96,
                    depths=(3, 3, 9, 3),
                    att_kernel=(11, 11, 11, 11),
